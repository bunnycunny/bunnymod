local orig_graaze = SniperGrazeDamage.on_weapon_fired
function SniperGrazeDamage:on_weapon_fired(weapon_unit, result)
	...
	for _,data in pairs(World:find_units_quick("all", managers.slot:get_mask("enemies"))) do
		local tied
		local surrender = data:brain():surrendered()
		
		if Network:is_server() then
			tied = data:brain()._logic_data.is_tied
			if tied and surrender then
				enemies_hit[data:key()] = false
			end
