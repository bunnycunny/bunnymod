tweak_data.upgrades.values.doctor_bag.amount_increase = {128}
if Network:is_server() then
	...
	if ray then
		...
		DoctorBagBase.spawn( pos, rot, amount_upgrade_lvl )
	end
	managers.mission._fading_debug_output:script().log('Doctor Spawn ACTIVATED', Color.green)
else
	if managers.hud and (managers.player:player_unit()) then
		managers.player:clear_equipment()
		managers.player._equipment.selections = {}
		managers.player:add_equipment({ silent = true, equipment = "doctor_bag" })
	end
	managers.mission._fading_debug_output:script().log(string.format("Doctor Bag - Equiped"), Color.green)
end