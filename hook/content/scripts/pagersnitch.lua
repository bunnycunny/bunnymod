	...
	function pager_snitcher:toggle()
		...
		if not global_pager_snitch_toggle then
			if not global_killall_on then --for not spam msg when kill loop is used
				managers.chat:_receive_message(1, "Snatcher", string.format("Player (client/host): true"), tweak_data.system_chat_color)
				managers.chat:_receive_message(1, "Snatcher", string.format("Team (host): true"), tweak_data.system_chat_color)
			end
		else
			if not global_killall_on then
				managers.chat:_receive_message(1, "Snatcher", string.format("Player (client/host): false"), tweak_data.system_chat_color)
				managers.chat:_receive_message(1, "Snatcher", string.format("Team (host): false"), tweak_data.system_chat_color)
			end
		end
