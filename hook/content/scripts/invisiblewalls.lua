...
global_invisible_walls = global_invisible_walls or false
if not global_invisible_walls then
	--Shot through walls
	...
	--msg
	managers.mission._fading_debug_output:script().log('ShotThroughWalls and WalkThroughWalls ACTIVATED',  Color.green)
else
	--Shot through walls
	for _,selection in pairs(player:inventory()._available_selections) do
		if selection.unit:base().old_mask then
			selection.unit:base()._bullet_slotmask = selection.unit:base().old_mask
			selection.unit:base().old_mask = nil
		end
	end
	--msg
	managers.mission._fading_debug_output:script().log('ShotThroughWalls DEACTIVATED',  Color.red)
end
global_invisible_walls = not global_invisible_walls